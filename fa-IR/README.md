...
من هر روز سوالاتی در مورد انتخاب جاوا اسکریپت در
[اینستاگرام](https://www.instagram.com/theavocoder)
ارسال می کنم و به نوبه خود آنها را در اینجا ارسال می کنم

آخرین به روز رسانی: <a href=#20200612><b>12 ژوئن</b></a>

از مقدماتی تا پیشرفته: از ساده تا پیچیده: خود را در جاوا اسکریپت آزمایش کنید، چیزهایی را که فراموش کرده اید به خاطر بسپارید یا خود را برای یک مصاحبه شغلی آینده آماده کنید! :muscle: :rocket: من این ریپازیتوری را به طور مرتب با سوالات جدید به روز می کنم.
...
```javascript
function* startGame() {
  const پاسخ = yield "Do you love JavaScript?";
  if (پاسخ !== "Yes") {
...
Every line is executed, until it finds the first `yield` keyword. There is a `yield` keyword on the first line within the function: the execution stops with the first yield! _This means that the variable `پاسخ` is not defined yet!_

When we call `game.next("Yes").value`, the previous `yield` is replaced with the value of the parameters passed to the `next()` function, `"Yes"` in this case. The value of the variable `پاسخ` is now equal to `"Yes"`. The condition of the if-statement returns `false`, and `JavaScript loves you back ❤️` gets logged.
